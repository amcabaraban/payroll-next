        ...
        for (const emp of employees) {
            try {
                ...
                const salary = await query(
                    'SELECT salary, salary_type FROM users WHERE id = ?',
                    [emp.id]
                );
                const empSalary = salary[0];
                const dailyRate = empSalary?.salary_type === 'monthly' 
                    ? (Number(empSalary.salary) * 12) / 365 
                    : Number(empSalary.salary);
