        ...
        return (
            <div className="flex min-h-screen bg-gray-100">
                ...
                <div className="flex-1">
                    <header className="shadow-sm p-4" style={{ background: 'linear-gradient(90deg, #097C87, #23CED9)' }}>
                    ...
                    <main className="p-6">
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-6">
                            <button onClick={() => router.push('/dashboard/attendance')}
                                ...
                                <p className="font-semibold mt-2">My DTR</p>
                                <p className="text-xs text-gray-400">View Attendance</p>
                            </button>
                            <button onClick={() => router.push('/dashboard/payslip')}
                                className="bg-white rounded-lg shadow p-6 hover:shadow-md transition text-center">
                                <span className="text-3xl">📄</span>
                                <p className="font-semibold mt-2">My Payslips</p>
                                <p className="text-xs text-gray-400">View Salary</p>
                            </button>
                            <div className="bg-white rounded-lg shadow p-6 text-center">
                                <span className="text-3xl">👤</span>
                                <p className="font-semibold mt-2">My Profile</p>
                                ...
                                <table className="w-full">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="p-3 text-right text-xs uppercase text-gray-500">Gross Pay</th>
                                            <th className="p-3 text-right text-xs uppercase text-gray-500">Deductions</th>
                                            <th className="p-3 text-right text-xs uppercase text-gray-500">Net Pay</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {myPayslips.map(p => (
                                            <tr key={p.id} className="border-b">
                                                <td className="p-3 text-sm">{formatDate(p.period_from)} - {formatDate(p.period_to)}</td>
                                                <td className="p-3 text-sm text-right">{formatPHP(p.gross_pay)}</td>
                                                <td className="p-3 text-sm text-right text-red-600">{formatPHP(p.total_deductions)}</td>
                                                <td className="p-3 text-sm text-right font-bold text-green-700">{formatPHP(p.net_pay)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                ...
                <main className="p-6">
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-6">
                        ...
                        <div className="bg-white rounded-lg shadow p-4">
                            ...
                            <p className="text-2xl font-bold text-gray-800">{employees.filter(e => e.role === 'admin').length}</p>
                        </div>
                        <div className="bg-white rounded-lg shadow p-4">
                        ...
                        <table className="w-full">
                            <thead className="bg-gray-50 border-b">
                                <tr>
                                    ...
                                    <th className="text-left p-3 text-xs font-medium text-gray-500 uppercase">Department</th>
                                    <th className="text-right p-3 text-xs font-medium text-gray-500 uppercase">Salary</th>
                                </tr>
                            </thead>
                            <tbody>
                                {employees.length === 0 ? (
                                    ...
                                    employees.map((emp) => (
                                        <tr key={emp.id} className="border-b hover:bg-gray-50">
                                            ...
                                            <td className="p-3 text-sm text-right font-medium">₱{Number(emp.salary).toLocaleString()}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
